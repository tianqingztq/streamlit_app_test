...
def fetch_all_patients():
    patients = []
    url = f"{FHIR_SERVER_URL}/Patient?_count=50"
...
def fetch_all_observations():
    observations = []
    url = f"{FHIR_SERVER_URL}/Observation?_count=50"
...
def fetch_all_medicationrequest():
    observations = []
    url = f"{FHIR_SERVER_URL}/MedicationRequest?_count=50"
...
def run_pcp_visulization():
    ...
    # Age Pie Chart
    with st.expander("Age", expanded=False):
    ...
    # Weight Histogram
    with st.expander("Weight", expanded=False):
        # Count occurrences of each weight range, including 'Unknown'
    ...
    with st.expander("A1C Test Results", expanded=False):
    ...
    with st.expander("Glucose Serum Test Results", expanded=False):
    ...
    with st.expander("Medication", expanded=False):
